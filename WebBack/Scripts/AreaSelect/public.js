(function () {
    ...
    $(".proCitySelAll").click(function (event) {
		...
		$(".provinceCityAll").css("top", 32).css("left", 8).toggle();
...
function viewHotCities() {
	...
	function(i, city) {
	    if (city.hotCity) {
	        ...
	        $(".hotCity .list ul").append("<li><a><input type='button' style='background:none;border:0px;cursor: pointer;' onclick=hotCityAddrInput(\'" + city.provinceId + "," + city.id + "," + city.name + "\') id='" + city.id + "' value='" +  cityname+ "'></a></li>");
		}
	});
}
...
function viewProvince(page) {
	...
	for (var i = start; i < end; i++) {
		...
		if (provinces[i].provinceName == '内蒙古') {
			p_name = '内蒙古';
		} else if (provinces[i].provinceName == '黑龙江') {
			p_name = '黑龙江';
		} else {
...
path = window['_AreaSelect_path'] || (function (script, i, me) {
    for (i in script) {
        // 如果通过第三方脚本加载器加载本文件，请保证文件名含有"AreaSelect"字符
        if (script[i].src && script[i].src.indexOf('AreaSelect') !== -1) me = script[i];
    };
...
function viewAllCities(i) {
	...
	if ($("#dimCityQuery").data("inputId") != null) {
	    ...
	    $("#" + selInputId + "Code").val(allProId);

	    if (typeof $("#" + selInputId + "Code").defaultPassed() != 'undefined') {
	        $("#" + selInputId + "Code").defaultPassed();
	    }
